...
def encoder(
    inputs, scope, levels, channels, number_of_units, type_of_layer, pool_strides
):
...
def transition_layer(net, input_scope, scope, levels, channels):
    with tf.variable_scope(scope):
        ...
        net[scope + "_level_" + str(levels - 1)] = net[
            input_scope + "_level_" + str(levels - 1)
        ]
    return net
    ...
    with tf.variable_scope(scope):
        ...
        for level in range(levels - 2, -1, -1):
            ...
            with tf.name_scope("level_" + str(level) + "/batch_norm"):
                net[scope + "_level_" + str(level)] = batch_norm(
                    net[scope + "_level_" + str(level)]
                )
...
def output_layer(net, input_scope, scope):
    with tf.variable_scope(scope):
        net[scope] = tf.layers.conv3d(
            inputs=net[input_scope + "_level_0"],
            filters=11,
            kernel_size=1,
            strides=1,
            padding="same",
        ...
        net[scope] = tf.nn.softmax(logits=net[scope], axis=4, name="softmax")
    return net


def get_layer(net, scope, type_of_layer, number_of_units, channels):
...
def batch_norm(net):
    net = tf.transpose(net, perm=[1, 2, 3, 0, 4])
    mean, var = tf.nn.moments(net, axes=[0, 1, 2])
    net = tf.nn.batch_normalization(
        net, mean=mean, variance=var, offset=0, scale=1, variance_epsilon=1e-9
    )
    net = tf.transpose(net, perm=[3, 0, 1, 2, 4])
    return net


def cnn_3d_segmentation(
    inputs, scope, levels, channels, encoder_units, decoder_units, pool_strides
):
    ...
    with tf.variable_scope(scope):
        ...
        net = output_layer(net=net, input_scope="decoder", scope="output")
    return net


def resnet_3d_segmentation(
    inputs, scope, levels, channels, encoder_units, decoder_units, pool_strides
):
    transition_channels = list((np.array(channels) * 0.25).astype(np.int32))
    with tf.variable_scope(scope):
