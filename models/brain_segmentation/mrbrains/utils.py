...
def get_loss(labels, predictions, loss_type, scope=None, **kwargs):
  ...
  """
    if loss_type == "absolute_difference":
        loss_func = lambda x, y, z: tf.losses.absolute_difference(
            labels=x, predictions=y, weights=z, reduction=tf.losses.Reduction.NONE
        )
    elif loss_type == "mean_squared_error":
        loss_func = lambda x, y, z: tf.losses.mean_squared_error(
            labels=x, predictions=y, weights=z, reduction=tf.losses.Reduction.NONE
        )
    elif loss_type == "log_loss":
        loss_func = lambda x, y, z: tf.losses.log_loss(
            labels=x, predictions=y, weights=z, reduction=tf.losses.Reduction.NONE
        )
    elif loss_type == "huber_loss":
        loss_func = lambda x, y, z: tf.losses.huber_loss(
            labels=x,
            predictions=y,
            weights=z,
...
def get_dsc(labels, predictions, scope=None):
    with tf.name_scope(scope, "dsc"):
        ...
        pred = tf.one_hot(
            indices=pred, depth=shape[-1], on_value=1, off_value=0, axis=-1
        )
...
def batch_norm_3d(inputs, name=None):
    with tf.name_scope(name, "batch_norm"):
        ...
        batch = tf.nn.batch_normalization(
            batch, mean=mean, variance=var, offset=0, scale=1, variance_epsilon=1e-9
        )
...
def re_arrange_array(array, new_shape, mode):
    if mode == "input":
        ...
        array = np.reshape(
            array,
            (int(shape[0] / (new_shape[3] / 24)), new_shape[3], 24, 8, new_shape[4]),
        )
        array = np.transpose(array, (0, 2, 1, 3, 4))
