...
class Subjects(object):
    ...
    def open_files(self):
        self.flair_img = sitk.ReadImage(
            os.path.join(self.path, self.name, "pre/FLAIR.nii.gz")
        )
        self.t1_img = sitk.ReadImage(
            os.path.join(self.path, self.name, "pre/reg_T1.nii.gz")
        )
        self.ir_img = sitk.ReadImage(
            os.path.join(self.path, self.name, "pre/reg_IR.nii.gz")
        )
        self.label_img = sitk.ReadImage(
            os.path.join(self.path, self.name, "segm.nii.gz")
        )
    ...
    def get_dsc(self):
        ...
        denom = np.sum(pred, axis=(0, 1, 2, 3)) + np.sum(
            self.label_array, axis=(0, 1, 2, 3)
        )
    ...
    def save_prediction(self, result_path):
...
def get_files(
    checkpoint="train/run_1/checkpoints/run_1",
    train_subjects=1,
    data_list=data_list,
    shuffle_files=True,
):
    try:
        ...
        files["train"] = [data_list[0]]
        files["val"] = [data_list[1]]
