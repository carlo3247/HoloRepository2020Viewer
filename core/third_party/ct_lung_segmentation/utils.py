...
def define_parameter():
    ...
    params[
        "airwayRadiusMask"
    ] = 15  # increase the value if you have high resolution image
    params["airwayRadiusX"] = 8  # ditto
    params["airwayRadiusZ"] = 15  # ditto
    params[
        "super2infer"
    ] = 0  # value = 1 if slice no. increases from superior to inferior, else value = 0
...
def close_space_dilation(
    ...
    if params["super2infer"]:
        Mtmp[
            initLoc[0] - Radius : initLoc[0] + Radius + 1,
    ...
    while totalChange > 0:
        ...
        for label in np.unique(L[:]):
            ...
            if label != 0 and np.sum(L[:] == label) > 10:
                ...
                xmin, xmax, ymin, ymax, zmin, zmax = bbox2_3D(
                    L, label, iterNoPerSlice, [m, n, p]
                )
                ...
                for iterCount in range(0, iterNoPerSlice):
                    Ltmp = ndimage.binary_dilation(
                        Ltmp, structure=struct_s, iterations=1
                    )
                    Ltmp = np.int8(Ltmp)
